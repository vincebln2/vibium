...
func TestRecordingRedactsEarlierObservationsAndOmitsSensitiveVisuals(t *testing.T) {
	...
	t.Setenv("ANTHROPIC_API_KEY", "ANTHROPIC-KEY-SENTINEL")
	t.Setenv("GOOGLE_API_KEY", "GOOGLE-KEY-SENTINEL")
	r := NewRecorder()
	...
	r.events = append(r.events,
		...
		recordEvent{"type": "after", "callId": "call@1", "result": map[string]interface{}{"status": "passed", "observation": "PASSWORD-SENTINEL and PROVIDER-KEY-SENTINEL and XAI-KEY-SENTINEL and ANTHROPIC-KEY-SENTINEL and GOOGLE-KEY-SENTINEL and LATE-HEADER-SECRET"}},
	)
	...
	for _, file := range z.File {
		...
		reader.Close()
		for _, secret := range []string{"PASSWORD-SENTINEL", "PROVIDER-KEY-SENTINEL", "XAI-KEY-SENTINEL", "ANTHROPIC-KEY-SENTINEL", "GOOGLE-KEY-SENTINEL", "LATE-HEADER-SECRET", "SECRET-IMAGE", "SECRET-DOM"} {
